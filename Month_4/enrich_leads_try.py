...
with open(config["output_file"], "w") as outfile:
    ...
    # If we have a checkpoint, skip until we find it
            if last_processed and not processing:
               if row['name'] == last_processed:
                  processing = True   # found it, now process next rows
               continue  
